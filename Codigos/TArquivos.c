...
int teste_lab(char *fileIN, char fileOUT){
    ...
    Stack *st;
    ponto p, at, vizP[4];
    int aux, nl, nc, i, num, flag=0;
    
    aux = open_file(fileIN, &matB);

    if (aux != SUCCESS){
        return ERROR;
    }

    nl_nc_mat(matB, &nl, &nc);
    matR = criar_mat(nl, nc);

    for(i=0; i<nl; i++){
        acessar_mat(matB, i, nc-1, &num);
        if(num==1)
            flag = 1;
    }
    if(flag==0){
    ...
    preencher_mat(matR, 0);
    st = stack_create();

    for(i=0; i<nl; i++){
        acessar_mat(matB, i, 0, &num);
        if(num==1){
            p.x=i;
            p.y=0;
            break;
    ...
    stack_push(st, p);
    escrever_mat(matR, p.x, p.y, 2);
    escrever_mat(matR, p.x, p.y+1, 2);
